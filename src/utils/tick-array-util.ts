...
export class TickArrayUtil {
  ...
  public static getTick(account: TickArrayData, tickIndex: number): TickData {
    invariant(TickArrayUtil.isValidTickIndex(tickIndex), "getTick - tick index out of range");

    const tickArrayIndex = TickArrayUtil.tickIndexToTickArrayIndex(account, tickIndex);
    invariant(
