...
export async function convertWhirlpoolDataToPoolData(
  ...
  for (const address of poolAddresses) {
    ...
    result[poolId] = {
      ...
      decimalFeeGrowthGlobalA: fromX64(pool.feeGrowthGlobalA).div(new Decimal(10).pow(decimalsA)),
      decimalFeeGrowthGlobalB: fromX64(pool.feeGrowthGlobalB).div(new Decimal(10).pow(decimalsB)),
